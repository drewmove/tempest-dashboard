...
def dashboard(request):
    ...
    print(f"currentime = {currentsample}")
    

    stations = WeatherStation.objects.all()
    for station in stations:
        print(f"lastsample = {station.lastsample}")
        timedelta = currentsample - station.lastsample
        print(f"delta = {timedelta} seconds")
        if currentsample - station.lastsample > 120:
            ...
            #WeatherStation.objects.update_or_create(
             #   station_id=station.station_id,
              ...
              #      'station_id': station.station_id,
              #      'name': station.name,
              #      'access_token': station.access_token,
